        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className={`glass-panel ${styles.authCard}`}>
          ...
          <form onSubmit={handleAuth} autoComplete="off" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div>
              ...
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  required
                  style={{ width: '100%', padding: '12px 46px 12px 16px', borderRadius: '12px', border: '1px solid var(--border)', fontSize: '15px', color: 'white', backgroundColor: 'rgba(255,255,255,0.03)', transition: 'all 0.2s', outline: 'none', boxSizing: 'border-box' }}
                  placeholder="••••••••"
                  onFocus={(e) => { e.target.style.borderColor = 'var(--primary)'; e.target.style.backgroundColor = 'rgba(255,255,255,0.05)'; }}
                  onBlur={(e) => { e.target.style.borderColor = 'var(--border)'; e.target.style.backgroundColor = 'rgba(255,255,255,0.03)'; }}
                />
            ...
            <button
              type="submit"
              disabled={loading}
              style={{ width: '100%', padding: '14px', background: 'linear-gradient(90deg, #818CF8, #4F46E5)', color: 'white', border: 'none', borderRadius: '12px', fontSize: '15px', fontWeight: '700', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1, transition: 'all 0.2s', boxShadow: '0 4px 14px 0 rgba(99, 102, 241, 0.39)', marginTop: '6px' }}
